...
const SERVER_ONLY_AGENT_ENV_KEY_PATTERN = /^PAPERCLIP_TOOL_OAUTH_[A-Z0-9_]+_CLIENT_SECRET$/;

function isServerOnlyAgentEnvKey(key: string): boolean {
  return SERVER_ONLY_AGENT_ENV_KEYS.has(key) || SERVER_ONLY_AGENT_ENV_KEY_PATTERN.test(key);
}
...
// Runtime API identity and server-only credentials are never accepted from
...
export function refreshPaperclipWorkspaceEnvForExecution(input: {
  ...
  for (const [key, value] of Object.entries(shapedEnvConfig)) {
    ...
    // config binding sets. Server-only credentials are always rejected.
