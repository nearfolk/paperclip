...
test("publish_package_to_npm uses trusted publishing from the bundled staging directory", () => {
  ...
  assert.match(
    result.calls,
    /^npx --yes npm@11\.18\.0 publish --ignore-scripts --tag canary --access public --loglevel verbose$/m,
  );
  assert.match(
    result.calls,
    /^npm publish --ignore-scripts --tag canary --access public --loglevel verbose$/m,
  );
...
test("publish_package_to_npm retries bundled directory tlog failures without provenance", () => {
  ...
  assert.match(
    result.calls,
    /^npm publish --ignore-scripts --tag canary --access public --provenance=false --loglevel verbose$/m,
  );
});
